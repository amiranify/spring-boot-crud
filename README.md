...
```
## Домашнее задание
CRUD-приложение необходимо модернизировать. У нашего пользователя
появилось несколько полей: пол и возраст.
1) Необходимо доработать наше приложение, чтобы мы смогли увидеть данную
информацию. Типы для отображения данных свойств требуется выбрать
самостоятельно.
2) Нужно доработать класс, который отражает данную сущность (User) и View для
пользователей.
